...
export default {
    async up(queryInterface) {
        const now = new Date();
        const userId = await queryInterface.sequelize.query(`SELECT id, fullname FROM users;`);
        const userIdMap = {};
        userId[0].forEach((user, index) => {
            userIdMap[user.fullname] = user.id;
        });
        const courseId = await queryInterface.sequelize.query(`SELECT id, title FROM courses;`);
        const courseIdMap = {};
        courseId[0].forEach((course, index) => {
            courseIdMap[course.title] = { id: course.id, title: course.title };
        });
        const tryoutId = await queryInterface.sequelize.query(`SELECT id, title FROM tryout_sections;`);
        const tryoutIdMap = {};
        tryoutId[0].forEach((tryout, index) => {
